...
export function AuthProvider({ children }) {
  ...
  const [phase, setPhase] = useState('loading'); // loading | guest | otp_pending | authenticated
  const [phone, setPhone] = useState('');
  const [otpMeta, setOtpMeta] = useState(null); // { hasAccount, emailSent, maskedEmail, devCode }
  ...
  // Step 2: Verify OTP code → creates session
  ...
  // Cancel OTP and go back to guest
  ...
  return (
    <AuthContext.Provider value={{
      ...
      lookupPhone, submitPhone, verifyOtp, resendOtp, cancelOtp,
