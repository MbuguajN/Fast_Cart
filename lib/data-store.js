...
const { writeJsonAtomic, mutateJson } = require('./atomic-json.js');
...
/**
 * Persist the store atomically (temp file + rename), so a crash mid-write
 * cannot truncate it and a reader never sees a partial file.
 */
function writeStore(data) {
  writeJsonAtomic(STORE_PATH, data);
}

/**
 * Serialised read-modify-write against the product store.
 *
 * Use for anything that adjusts a running value — stock decrements from the
 * order webhook above all, where two concurrent orders would otherwise each
 * read the same starting quantity and one decrement would be lost.
 */
function mutateStore(mutate) {
  return mutateJson(STORE_PATH, readStore, writeStore, mutate);
}
...
function isProductInStock(p) {
  ...
  if (p.stockQuantity !== null && p.stockQuantity !== undefined && p.stockQuantity !== '') {
    const qty = Number(p.stockQuantity);
    if (!isNaN(qty) && qty <= 0 && p.type !== 'variable') return false;
  }
  if (p.type === 'variable' && p.variations && p.variations.length > 0) {
    const hasInStockVar = p.variations.some(v => 
      v.stockStatus === 'instock' && 
      (v.stockQuantity === null || v.stockQuantity === undefined || v.stockQuantity === '' || Number(v.stockQuantity) > 0)
    );
    if (!hasInStockVar) return false;
  }
  return true;
}
...
/**
 * `zone.zonePrice`/`zone.locations` — matching the shape lib/shipping.js
 * reads back via getZoneCatalog(). This used to destructure `zone.price`/
 * `zone.keywords`, which no caller (the WooCommerce sync, or the admin
 * zones page) ever sends — every upsert silently stored `{price: 300,
 * keywords: []}` regardless of input, and every zone collapsed to a flat
 * 300 with no per-location pricing the moment a sync ran.
 */
function upsertZone(zone) {
  ...
  const entry = {
    id: zone.id,
    name: zone.name,
    zonePrice: zone.zonePrice || 300,
    locations: Array.isArray(zone.locations)
      ? zone.locations.map((loc) => ({
          name: loc.name,
          keywords: Array.isArray(loc.keywords) ? loc.keywords : [],
          price: loc.price,
        }))
      : [],
...
function upsertProduct(wcProduct) {
  ...
  const standardSku = generateStandardSku(wcProduct);
  const entry = {
    wcId: wcProduct.wcId,
    name: wcProduct.name,
    slug: wcProduct.slug,
  ...
  // Drop every field the caller did not actually supply.
  //
  // `entry` names all fields and defaults absent ones to '', [] or null, so
  // without this a partial update — a webhook stock delta, say — overwrites
  // good stored values with blanks. Keyed off the source payload rather than
  // the built entry, because those fallbacks hide the absence.
  const COMPUTED_KEYS = new Set(['variations', 'lastSynced', 'overrides']);
  for (const key of Object.keys(entry)) {
    if (COMPUTED_KEYS.has(key)) continue;
    if (wcProduct[key] === undefined) delete entry[key];
  }
  ...
  // An overrides-only write must not blank the commerce fields. Detect it by
  // asking whether the caller supplied any WooCommerce-owned field at all,
  // rather than by counting keys.
  const carriesWcFields = WOOCOMMERCE_OWNED_FIELDS.some(
    (field) => wcProduct[field] !== undefined
  );
  if (!carriesWcFields && existing) {
