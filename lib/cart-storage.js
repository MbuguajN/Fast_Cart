...
export function addLine(lines, rawId, variantId = null) {
  const id = typeof rawId === 'object' && rawId !== null ? Number(rawId.id || rawId.wcId) : Number(rawId);
  if (!Number.isFinite(id)) return lines;
...
export function changeQty(lines, rawId, delta) {
  const id = typeof rawId === 'object' && rawId !== null ? Number(rawId.id || rawId.wcId) : Number(rawId);
  if (!Number.isFinite(id)) return lines;
  return lines
...
export function removeLine(lines, rawId) {
  const id = typeof rawId === 'object' && rawId !== null ? Number(rawId.id || rawId.wcId) : Number(rawId);
  if (!Number.isFinite(id)) return lines;
