...
function forbidden(message = 'Insufficient permissions for this area') {
  return NextResponse.json({ error: message }, { status: 403 });
}

/**
 * Which admin_staff role a path requires. Centralized here (rather than in
 * every route handler) so every existing and future adminGuard()/
 * requireAdminOr401() call gets scope enforcement automatically — an admin
 * route that forgets to check role can't happen, because the check isn't
 * theirs to forget.
 *
 * 'owner' always passes every check (see hasRequiredScope). Shared utility
 * endpoints (auth, upload, health) accept any authenticated staff role —
 * they're not where retail/trade separation matters.
 */
function requiredScopeForPath(pathname) {
  if (pathname.startsWith('/api/admin/staff')) return 'owner';
  if (pathname.startsWith('/api/admin/trade')) return 'trade';
  if (
...
function hasRequiredScope(user, pathname) {
  const required = requiredScopeForPath(pathname);
  if (required === 'any') return true;
  if (user.role === 'owner') return true;
  return user.role === required;
}

/** Requires a valid `admin_token` cookie, scoped to the requested area. Returns null when allowed. */
export async function adminGuard(request) {
  const user = await requireAdmin(request);
  if (!user) return unauthorized();
  if (!hasRequiredScope(user, request.nextUrl.pathname)) return forbidden();
  return null;
}
...
/**
 * Resolves the admin identity, or returns a 401/403 response. Scoped the
 * same way adminGuard() is — see requiredScopeForPath.
...
export async function requireAdminOr401(request) {
  ...
  if (!hasRequiredScope(user, request.nextUrl.pathname)) return { user: null, denied: forbidden() };
