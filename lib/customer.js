...
export async function findOrCreateCustomer({ phone, name, email, landmark, zone }) {
  ...
  if (existing) {
    ...
    // Any explicitly-submitted landmark updates the address on file — not
    // just a first-time fill-in. This used to only apply when the customer
    // had no address at all, which meant a returning customer's delivery
    // zone was permanently stuck at whatever their first order used: the
    // location picker could compute a fresh, correct zone, but the profile
    // that checkout restores from would keep serving the old one forever.
    if (landmark) {
      ...
      // Replace rather than append: meta_data pushed without a matching
      // `id` is added as a new entry by the WooCommerce REST API, so
      // repeatedly pushing here without first dropping the old entry would
      // accumulate duplicate landmark_hint/delivery_zone rows — and
      // `Array.find` in /api/auth/session would then keep returning the
      // *first* (oldest) one, silently undoing every later location update.
      const baseMeta = (updatePayload.meta_data || existing.meta_data || [])
        .filter((m) => m.key !== 'landmark_hint' && m.key !== 'delivery_zone');
      updatePayload.meta_data = baseMeta;
