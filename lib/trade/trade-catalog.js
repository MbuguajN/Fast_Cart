 ...
 * `{ prkCostIncVat: 1, priceLine: 'jaba' }` and price the order at a cost
 * base they chose.
 *
 ...
 *
 * Spirits carry no licence gate here — any buyer, guest or account, may
 * order them; age is confirmed once at checkout instead (see checkout
 * route). A liquor licence is only collected for the credit/partner track.
 *
 ...
 * @throws {Error} on an unknown SKU, bad quantity, or insufficient stock
 */
export async function resolveTradeLineItems(items, { checkStock = false } = {}) {
