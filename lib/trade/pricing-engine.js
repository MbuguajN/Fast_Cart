...
export const DEFAULT_PRICE_BANDS = {
  spirits: {
    ...
    bands: [
      { key: 'T0', min: 1, max: 5, markup_on_prk_incvat: 0.15 },
  ...
  jaba: {
    basis: 'per_sku_per_order',
    vat_mode: 'inclusive', // Standardized to match spirits — displayed, edited and charged VAT-inclusive (16% on the original 800/750/700/650/600 ex-VAT list).
    bands: [
      { key: 'T0', min: 1, max: 10, price_inc_vat: 928 },
      { key: 'T1', min: 11, max: 50, price_inc_vat: 870 },
      { key: 'T2', min: 51, max: 100, price_inc_vat: 812 },
      { key: 'T3', min: 101, max: 200, price_inc_vat: 754 },
      { key: 'T4', min: 201, max: null, price_inc_vat: 696 },
    ],
  },
};
...
export const JABA_MIN_ORDER_BOTTLES = 10; // Jaba-only orders: 10 bottles, not 12
...
/** Converts a jaba ex-VAT price to inc-VAT, in exactly one rounded step — the
 * single source of truth so setPriceOverride and calculateTradeOrderPricing
 * can never compute a different inc-VAT figure for the same ex-VAT input. */
export function jabaExVatToIncVat(priceExVat) {
  return roundCent(priceExVat * (1 + VAT_RATE));
}
...
export function resolveLineTier({
  ...
  if (priceLine === 'jaba') {
    ...
    // Standardized to the spirits convention: inc-VAT is the quoted figure,
    // ex-VAT is derived from it — not the other way around.
    const unitPriceIncVat = roundKes(band.price_inc_vat);
    const unitPriceExVat = roundCent(unitPriceIncVat / (1 + VAT_RATE));
    const vatAmountPerUnit = roundCent(unitPriceIncVat - unitPriceExVat);
...
export function calculateUpgradeNudge({
  ...
  if (priceLine === 'jaba') {
    ...
    const neededQty = nextBand.min - qty;
    const nextPriceIncVat = roundKes(nextBand.price_inc_vat);
    const currentPriceIncVat = currentRes.unitPriceIncVat;
    const savingsPerBottle = Math.max(0, currentPriceIncVat - nextPriceIncVat);
 ...
 *
 * Either threshold alone clears the minimum — 12 bottles across all SKUs,
 * OR KES 10,000 of goods value, whichever the buyer reaches first. This is
 * an OR, not an AND: a small basket of premium bottles can qualify on value
 * alone before hitting 12 bottles, and a large basket of cheap bottles can
 * qualify on volume alone before hitting KES 10,000.
 *
 * A Jaba-only order (no spirits lines at all) uses a lower 10-bottle floor
 * instead of 12 — the KES 10,000 value alternative is unchanged. A mixed
 * cart with any spirits line still uses the general 12-bottle floor.
 */
export function evaluateMinimumOrderRule({ totalBottles, goodsValueExVat, isJabaOnly = false }) {
  const bottles = parseInt(totalBottles, 10) || 0;
  const valueExVat = roundCent(goodsValueExVat || 0);
  const minBottles = isJabaOnly ? JABA_MIN_ORDER_BOTTLES : MIN_ORDER_BOTTLES;

  const meetsBottleCondition = bottles >= minBottles;
  const meetsValueCondition = valueExVat >= MIN_ORDER_GOODS_EX_VAT;

  const passed = meetsBottleCondition || meetsValueCondition;

  const bottleDeficit = Math.max(0, minBottles - bottles);
  const valueDeficit = Math.max(0, MIN_ORDER_GOODS_EX_VAT - valueExVat);

  // passed is only false when BOTH conditions are unmet, since either one
  // alone is sufficient — so the message always offers both remaining paths.
  // Kept to one line; the "what this is measured against" explanation lives
  // in the UI's progress breakdown (see cart page), not repeated here.
  const message = passed
    ? ''
    : `Add ${bottleDeficit} more bottle${bottleDeficit > 1 ? 's' : ''} OR KES ${valueDeficit.toLocaleString()} more goods value — whichever comes first.`;
...
export function calculateTradeOrderPricing({
  ...
  const processedLines = items.map((item) => {
    ...
    if (res.eligible && override !== undefined && override !== null) {
      ...
      // Standardized: an override is always typed and stored inc-VAT, for
      // both price lines — Jaba no longer has a separate ex-VAT input
      // convention (see DEFAULT_PRICE_BANDS.jaba).
  ...
  const isJabaOnly = processedLines.length > 0 && processedLines.every((l) => l.priceLine === 'jaba');
