  ...
  const processedLines = items.map((item) => {
    ...
    if (res.eligible && override !== undefined && override !== null) {
      ...
      if (priceLine === 'jaba') {
        ...
        // Single source of truth for the ex-VAT -> inc-VAT conversion (see
        // jabaExVatToIncVat) — setPriceOverride uses the same function, so
        // the two paths can never disagree on unitPriceIncVat for the same
        // input. vatAmountPerUnit is derived from that one rounded figure,
        // not from a separately-rounded multiplication.
        unitPriceIncVat = jabaExVatToIncVat(unitPriceExVat);
      ...
      // Margin is always computed inc-VAT price vs inc-VAT landed cost,
      // regardless of price line — the override *input* convention (ex-VAT
      // for jaba, inc-VAT for spirits) only governs how the typed number
      // converts to unitPriceIncVat above. Comparing an ex-VAT price to the
      // inc-VAT landed cost directly would understate margin by the VAT
      // amount.
