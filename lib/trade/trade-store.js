...
export function importPrkCostsCsv(csvContent, isDryRun = true, user = 'Admin') {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const oldTierPrices = computeSpiritsTierPrices(oldCost, bands);
    const newTierPrices = computeSpiritsTierPrices(newCost, bands);
    ...
    diffs.push({
      ...
      newTierPrices,
      deltaTierPrices: {
        T1: newTierPrices.T1 - oldTierPrices.T1,
        T2: newTierPrices.T2 - oldTierPrices.T2,
        T3: newTierPrices.T3 - oldTierPrices.T3,
      },
    });
...
function mapQuoteRecord(r, itemRows = []) {
  return {
    ...
    items: itemRows.map((it) => {
      ...
      const unitCost = snapshot > 0 ? snapshot / qty : Number(it.unit_price_inc_vat) * 0.75;
      return {
        id: it.product_id,
        sku: it.sku,
        name: it.name,
...
export async function getTradeProducts(filters = {}) {
  ...
  return res.rows.map((r) => {
    ...
    return {
      ...
      tierPrices: r.price_line === 'spirits' ? computeSpiritsTierPrices(cost, bands) : null,
    };
  });
}
...
export async function updateTradeProduct(skuOrId, patch, user = 'Admin') {
  ...
  return withTransaction(async (client) => {
    ...
    const newCost = patch.prkCostIncVat !== undefined ? Number(patch.prkCostIncVat) : Number(existing.prk_cost_inc_vat);

    await client.query(
      'UPDATE trade_products SET stock_quantity = $1, prk_cost_inc_vat = $2, in_stock = ($1 > 0), updated_at = NOW() WHERE id = $3',
      [newQty, newCost, existing.id]
    );
...
export function setTradeUserPasswordHash(userId, passwordHash, { mustChange = false } = {}) {
  return mutateTradeStore((store) => {
    ...
    // Keep PostgreSQL trade_users in sync
    query(
      'UPDATE trade_users SET password_hash = $1, must_change_password = $2, updated_at = NOW() WHERE id = $3',
      [passwordHash, mustChange, userId]
    ).catch((err) => console.error('Failed to sync password hash to Postgres:', err.message));
