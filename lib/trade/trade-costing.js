...
import { roundCent, calculateTradeOrderPricing, jabaExVatToIncVat } from './pricing-engine.js';
...
export async function setPriceOverride({ productId, tierKey, priceLine, price, updatedBy }) {
  ...
  //
  // Round to cents *before* calling jabaExVatToIncVat, matching
  // pricing-engine.js's override branch (which converts `unitPriceExVat =
  // roundCent(override)`, not the raw override, to inc-VAT). Without this,
  // a fractional-cent `price` (more than 2 decimals — e.g. from an
  // un-rounded caller) would still make setPriceOverride and
  // calculateTradeOrderPricing disagree on unitPriceIncVat by up to 0.01,
  // even though both now call the same jabaExVatToIncVat function — the
  // shared function alone only guarantees agreement when both call sites
  // feed it the same *already-rounded* ex-VAT figure.
  const priceIncVat = priceLine === 'jaba' ? jabaExVatToIncVat(roundCent(priceNum)) : priceNum;
