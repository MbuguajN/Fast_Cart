...
import { roundCent, roundKes, calculateTradeOrderPricing } from './pricing-engine.js';
...
export function getMarginFloorConfig() {
  ...
  const mf = store.config?.marginFloor || {};
  return {
    spirits: Number.isFinite(Number(mf.spirits)) ? Number(mf.spirits) : legacy,
    jaba: Number.isFinite(Number(mf.jaba)) ? Number(mf.jaba) : legacy,
  };
}
...
export async function setPriceOverride({ productId, tierKey, priceLine, price, updatedBy }) {
  ...
  // Standardized: the override input is inc-VAT for both price lines now
  // (Jaba no longer has a separate ex-VAT input convention — see
  // pricing-engine.js's DEFAULT_PRICE_BANDS.jaba), so it compares directly
  // against landed cost, which is always inc-VAT. Rounded to whole KES
  // here, once, because calculateTradeOrderPricing's override branch reads
  // this stored value straight through `roundKes(override)` — storing the
  // unrounded figure would let this function's own margin/status disagree
  // with what an order actually gets charged for a fractional-cent input.
  const priceIncVat = roundKes(priceNum);

  // A positive-but-sub-50-cent price (e.g. 0.01) rounds to 0 here — that's
  // not "no margin data", it's a price that can't be charged at all, so it
  // must classify as squarely blocked rather than silently reading as a 0%
  // margin (which classifyMarginStatus would only flag, not block).
  const marginPercent = priceIncVat > 0 ? roundCent(((priceIncVat - landedCost) / priceIncVat) * 100) : -100;
  ...
  await query(
    ...
    [productId, tierKey, priceIncVat, updatedBy || 'Admin']
  );
...
export async function recordStockReceipt({
  ...
  if (new Set(skus).size !== skus.length) {
    throw new Error('A stock receipt cannot contain two lines for the same SKU — combine them into one line');
  }
  for (const line of lines) {
    if (!(Number(line.cases) > 0)) {
      throw new Error(`Receipt line for SKU "${line.sku}" must have a positive case quantity`);
    }
    if (!(Number.isFinite(Number(line.unitProductCost)) && Number(line.unitProductCost) > 0)) {
      throw new Error(`Receipt line for SKU "${line.sku}" must have a positive product cost`);
    }
  }
  ...
  return withTransaction(async (client) => {
    ...
    for (let i = 0; i < allocated.length; i++) {
      ...
      const existingProductCost = Number(product.product_cost_inc_vat) || existingCost;
      ...
      const newProductCost = computeWeightedAverageCost({
        existingQty, existingCost: existingProductCost,
        newQty: line.bottles, newCost: line.unitProductCost,
      });
