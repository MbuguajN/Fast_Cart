...
import { sendTradeInvoiceEmail } from '@/lib/trade/trade-email.js';
...
export async function POST(request, { params }) {
  ...
  try {
    const rl = await rateLimitRequest(request, { scope: '/api/trade/invoices/email', maxRequests: 10, windowMs: 300000 });
    if (!rl.allowed) {
    ...
    await sendTradeInvoiceEmail({ to: recipientEmail, invoice, customNotes });

    return NextResponse.json({ success: true });
  } catch (error) {
    console.error('Trade invoice email failed:', error.message);
    return NextResponse.json({ error: 'Failed to email invoice. Please try again.' }, { status: 500 });
  }
}
