...
function OrdersContent() {
  ...
  const loadOrders = useCallback(async () => {
    setLoading(true);
    try {
      // Identity comes from the session cookie — only the date filters are
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col justify-between">
      {/* Top Header Nav */}
      <header className="sticky top-0 w-full z-50 shadow-md bg-[#840037] text-white">
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-8 pt-6 pb-16 w-full flex-1">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
          ...
          <div className="md:col-span-4 space-y-4">
            <div className="bg-white rounded-2xl p-5 border border-gray-200 shadow-xs space-y-4">
              ...
              {/*
                The phone lookup that used to sit here has been removed: it
                queried any customer's order history by number. Order history
                is now scoped to the signed-in account.
              */}
              <p className="text-xs text-gray-600 leading-relaxed">
                Showing orders for{' '}
                <span className="font-semibold text-gray-900">{user?.phone ? `0${String(user.phone).slice(-9)}` : 'your account'}</span>.
              </p>
